...
const ProviderLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      if (result.user.userType !== 'provider') {
        setError('This is not a provider account. Please use the Client Portal to login.');
        setLoading(false);
        return;
      }
  ...
  const handleGoogleLogin = () => {
    // Store intended user type in sessionStorage before redirecting
    sessionStorage.setItem('intended_user_type', 'provider');
    sessionStorage.removeItem('invite_code'); // Clear any invite code
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start space-x-2">
              ...
              />
              <div>
                <Label htmlFor="keepLoggedIn" className="text-sm cursor-pointer">
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <Link to="/provider/register" className="text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium">
              {t('auth.registerAsProvider')}
            </Link>
          </div>

          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 text-center">
            <span className="text-gray-500 dark:text-gray-400 text-sm">{t('auth.areYouPatient')} </span>
            <Link to="/client/login" className="text-green-600 hover:text-green-700 dark:text-green-400 font-medium text-sm">
              {t('auth.goToClientPortal')} →
            </Link>
          </div>
